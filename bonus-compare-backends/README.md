...
Neither Codex CLI nor opencode has a skill/slash-command mechanism
identical to Claude Code's, so there's no `backend-compare` command to
invoke directly. Reach the same outcome by asking your agent to do the
